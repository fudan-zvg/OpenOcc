...
class MultiGrid(nn.Module):
    def __init__(self, sizes, occ_feature_dim):
        super().__init__()
        volumes = []
        self.sizes = sizes

        for i, size in enumerate(sizes):
            init_val = torch.rand(1, occ_feature_dim[i], 1, 1, 1) * 2. - 1.
    ...
    def forward_i(self, grid, align_corners=True, apply_smoothstep=False, scale = None):
        volume = self.volumes[scale]
        out = SmoothSampler.apply(volume, grid, "border", align_corners, apply_smoothstep)
        return out
